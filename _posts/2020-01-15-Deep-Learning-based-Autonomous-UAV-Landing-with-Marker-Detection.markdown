...
The above chart displays the evolution of the mean average precision (`mAP`) together with the average loss during the training. One can observe that the network was trained for 2000 epochs (iteration)
...
A good part of the project involved training Tiny YOLO with respect to the ground marker. For this reason a set of images including the ground marker were acquired from the simulator. The images are used to train the deep neural network Tiny Yolo. The main tools and framework used during the development are as follows 
